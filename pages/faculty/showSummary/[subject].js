...
function  AttendanceSummaryPage() {
  ...
  const filteredStudents = studentsData.filter((student) => {
    ...
    const withinDateRange =
      (!start || studentDate >= start) && (!end || studentDate <= end);

    const withinPercentRange = percentFilter
      ? student.percent >= parseInt(percentFilter)
      : true;
    ...
    <div className="flex flex-col items-center justify-center bg-gray-100 p-4 sm:p-6">
      <div className="w-full max-w-4xl bg-white p-4 sm:p-6 rounded-lg shadow-md">
        ...
        <div className="mb-4 sm:mb-6 flex flex-col sm:flex-row justify-between gap-4">
          <div className="flex flex-col sm:flex-row gap-4">
            <div>
              ...
              <input
                ...
                className="w-full sm:w-auto px-4 py-2 text-sm border rounded-md focus:outline-none focus:ring-2 focus:ring-blue-400"
              />
            </div>
            <div>
          ...
          {/* Percent Filter */}
          <div>
            <label className="block text-sm text-gray-600">Min Percent</label>
            <input
              type="number"
              min="0"
              max="100"
          ...
          <table className="w-full table-auto border-collapse">
            <thead>
              <tr className="bg-gray-200 text-gray-700">
                <th className="px-2 sm:px-4 py-2 text-left border">Index</th>
                <th className="px-2 sm:px-4 py-2 text-left border">Name</th>
                <th className="px-2 sm:px-4 py-2 text-left border">Enrollment</th>
                <th className="px-2 sm:px-4 py-2 text-left border">Percent</th>
              </tr>
            </thead>
            <tbody>
              {filteredStudents.length > 0 ? (
                filteredStudents.map((student) => (
                  <tr key={student.index} className="border-b">
                    <td className="px-2 sm:px-4 py-2 text-gray-600">{student.index}</td>
                    <td className="px-2 sm:px-4 py-2 text-gray-600">{student.name}</td>
                    <td className="px-2 sm:px-4 py-2 text-gray-600">{student.enrollment}</td>
                    <td className="px-2 sm:px-4 py-2 text-gray-600">{student.percent}%</td>
                  </tr>
                ))
              ) : (
                <tr>
